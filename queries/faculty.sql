...
DROP FUNCTION IF EXISTS how_many_faculty(deck text, hour text);

CREATE FUNCTION how_many_faculty(deck text, hour text)
RETURNS TABLE(deck text, time_stamp text, occupied_spaces integer, available_spaces integer) AS $$
 ...
 FROM garage
  ...
  WHERE garage.zone_id = 2
    ...
    AND garage.deck = $1
    AND SUBSTRING(time_stamp, 12, 2) = $2
GROUP BY garage.deck, o.time_stamp, o.occupied
ORDER BY time_stamp, deck

$$ LANGUAGE SQL STABLE STRICT;

ALTER FUNCTION how_many_faculty(time_of_day timestamp) OWNER TO parking;